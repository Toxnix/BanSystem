package de.bacherik.bansystem.platform.bungee;

import de.bacherik.bansystem.platform.bungee.commands.*;
import de.bacherik.bansystem.platform.bungee.listeners.PreLogin;
import de.bacherik.bansystem.platform.bungee.listeners.PreSendMessage;
...
public final class Main extends Plugin {
    ...
    public void onEnable() {
        ...
        DBConfig dbConfig = new DBConfig("database.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);
        settingsConfig = new SettingsConfig("settings.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);

        MessagesConfig germanConfig = new MessagesGermanConfig("messages_german.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);
        MessagesConfig englishConfig = new MessagesEnglishConfig("messages_english.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);
        ...
        sql = new MySQL(dbConfig.getHost(), dbConfig.getPort(), dbConfig.getUsername(), dbConfig.getPassword(), dbConfig.getDatabase(), ServerSoftware.BUNGEECORD);
    ...
    public void reloadConfigs() {
        ...
        if (settingsConfig.getLanguage().equalsIgnoreCase("german")) {
            msgsConfig = new MessagesGermanConfig("messages_german.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);
        } else if (settingsConfig.getLanguage().equalsIgnoreCase("english")) {
            msgsConfig = new MessagesEnglishConfig("messages_english.yml", "plugins/BanSystem", ServerSoftware.BUNGEECORD);
        }
    }
                ...
                while (true) {
                    ...
                    if (params.length == 6 && cmd.equals("ban")) {
                        ...
                        Util.broadcastMessage(msgsConfig.get("bansystem.ban.successful")
                                ...
                                "bansystem.ban", ServerSoftware.BUNGEECORD);
                        ...
                        Util.broadcastMessage(Main.getInstance().getMessagesConfig().get("bansystem.unban.successful")
                                        ...
                                        .replaceAll("%REASON%", reason),
                                "bansystem.unban", ServerSoftware.BUNGEECORD);
                    } else {
